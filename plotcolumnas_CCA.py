...
n_CO2 = "1204-1406_CO2_CCA.txt"
n_O2 = "1204-1406_O2_CCA.txt"

######################################################################
####################	READING WITH DTYPE FORMAT
dt_txt = np.dtype([('fname',np.str_,30),('epochTime',int),('SZA',float), \
('totCol',float),('RMS',float),('relRMS',float),('wnShift',float),('TOTerr',float)])
...
# CO2
hist_rr_CO2,be_rr_CO2 = np.histogram(f_CO2['relRMS'],bins=75)
bp_rr_CO2 = np.array([(ele + be_rr_CO2[ii+1])/2.0 for ii,ele in enumerate(be_rr_CO2[:-1])])
cum_rr_CO2 = [hist_rr_CO2[0]]
for ww,wlw in enumerate(hist_rr_CO2[1:]):
	cum_rr_CO2.append(wlw + cum_rr_CO2[ww])
hist_ws_CO2,be_ws_CO2 = np.histogram(f_CO2['wnShift'],bins=75)
bp_ws_CO2 = np.array([(ele + be_ws_CO2[ii+1])/2.0 for ii,ele in enumerate(be_ws_CO2[:-1])])
cum_ws_CO2 = [hist_ws_CO2[0]]
for qq,qlq in enumerate(hist_ws_CO2[1:]):
	cum_ws_CO2.append(qlq + cum_ws_CO2[qq])
hist_te_CO2,be_te_CO2 = np.histogram(f_CO2['TOTerr'],bins=75)
bp_te_CO2 = np.array([(ele + be_te_CO2[ii+1])/2.0 for ii,ele in enumerate(be_te_CO2[:-1])])
cum_te_CO2 = [hist_te_CO2[0]]
for mm,mlm in enumerate(hist_te_CO2[1:]):
	cum_te_CO2.append(mlm + cum_te_CO2[mm])
dt_CO2 = np.array([dt.datetime.fromtimestamp(ele) for ele in f_CO2['epochTime']])		### DATETIME FOR CO2

plt.figure()
plt.suptitle('Relative RMS CO2')
plt.subplot(211)
plt.plot(dt_CO2,f_CO2['relRMS'],'.r',label='Relative RMS CO2')
plt.ylabel('Relative RMS CO2')
plt.subplot(212)
#plt.plot(bp_rr_CO2,hist_rr_CO2,'-b',label='Relative RMS CO2')
plt.plot(bp_rr_CO2,cum_rr_CO2,'--g',label='Relative RMS CO2')
plt.ylabel('Histogram\nRelative RMS CO2')
plt.figure()
plt.suptitle('Wavenumber Shift CO2')
ax_ws = plt.subplot(211)
plt.plot(dt_CO2,f_CO2['wnShift'],'.r',label='Wavenumber Shift CO2')
plt.ylabel('Wavenumber Shift CO2')
ax_h_ws = plt.subplot(212)
plt.plot(bp_ws_CO2,hist_ws_CO2,'-b',label='Wavenumber Shift CO2')
#plt.plot(bp_ws_CO2,cum_ws_CO2,'--g',label='Wavenumber Shift CO2')
plt.ylabel('Histogram\nWavenumber Shift CO2')
plt.figure()
plt.suptitle('Total Error CO2')
plt.subplot(211)
plt.plot(dt_CO2,f_CO2['TOTerr'],'.r',label='Total Error CO2')
plt.ylabel('Total Error CO2')
plt.subplot(212)
plt.plot(bp_te_CO2,hist_te_CO2,'-b',label='Total Error CO2')
plt.plot(bp_te_CO2,cum_te_CO2,'--g',label='Total Error CO2')
plt.ylabel('Histogram\nTotal Error CO2')

# O2
hist_rr_O2,be_rr_O2 = np.histogram(f_O2['relRMS'],bins=75)
bp_rr_O2 = np.array([(ele + be_rr_O2[ii+1])/2.0 for ii,ele in enumerate(be_rr_O2[:-1])])
cum_rr_O2 = [hist_rr_O2[0]]
for aa,ala in enumerate(hist_rr_O2[1:]):
	cum_rr_O2.append(ala + cum_rr_O2[aa])
hist_ws_O2,be_ws_O2 = np.histogram(f_O2['wnShift'],bins=75)
bp_ws_O2 = np.array([(ele + be_ws_O2[ii+1])/2.0 for ii,ele in enumerate(be_ws_O2[:-1])])
cum_ws_O2 = [hist_ws_O2[0]]
for zz,zlz in enumerate(hist_ws_O2[1:]):
	cum_ws_O2.append(zlz + cum_ws_O2[zz])
hist_te_O2,be_te_O2 = np.histogram(f_O2['TOTerr'],bins=75)
bp_te_O2 = np.array([(ele + be_te_O2[ii+1])/2.0 for ii,ele in enumerate(be_te_O2[:-1])])
cum_te_O2 = [hist_te_O2[0]]
for tt,tlt in enumerate(hist_te_O2[1:]):
	cum_te_O2.append(tlt + cum_te_O2[tt])
dt_O2 = np.array([dt.datetime.fromtimestamp(ele) for ele in f_O2['epochTime']])			### DATETIME FOR O2

plt.figure()
plt.suptitle('Relative RMS O2')
plt.subplot(211)
plt.plot(dt_O2,f_O2['relRMS'],'.r',label='Relative RMS O2')
plt.ylabel('Histogram\nRelative RMS O2')
plt.subplot(212)
#plt.plot(bp_rr_O2,hist_rr_O2,'-b',label='Relative RMS O2')
plt.plot(bp_rr_O2,cum_rr_O2,'--g',label='Relative RMS O2')
plt.ylabel('Histogram\nRelative RMS O2')
plt.figure()
plt.suptitle('Wavenumber Shift O2')
plt.subplot(211,sharex = ax_ws)
plt.plot(dt_O2,f_O2['wnShift'],'.r',label='Wavenumber Shift O2')
plt.ylabel('Wavenumber Shift O2')
plt.subplot(212,sharex = ax_h_ws)
plt.plot(bp_ws_O2,hist_ws_O2,'-b',label='Wavenumber Shift O2')
#plt.plot(bp_ws_O2,cum_ws_O2,'--g',label='Wavenumber Shift O2')
plt.ylabel('Histogram\nWavenumber Shift O2')
plt.figure()
plt.suptitle('Total Error O2')
plt.subplot(211)
plt.plot(dt_O2,f_O2['TOTerr'],'.r',label='Total Error O2')
plt.ylabel('Total Error O2')
plt.subplot(212)
plt.plot(bp_te_O2,hist_te_O2,'-b',label='Total Error O2')
plt.plot(bp_te_O2,cum_te_O2,'--g',label='Total Error O2')
plt.ylabel('Histogram\nTotal Error O2')
...
con_CO2_I = ((f_CO2['relRMS'] > 0.0) & (f_CO2['relRMS'] < 0.36)) & \
((f_CO2['wnShift'] > -5.0e-6) & (f_CO2['wnShift'] < 1.5e-5)) & \
((f_CO2['TOTerr'] < 1.0e20))
f_CO2 = f_CO2[con_CO2_I]
dt_CO2 = dt_CO2[con_CO2_I]
print 'FILTER #1 CO2', f_CO2.shape
# 2ND FILTER CO2
mn_CO2 = np.mean(f_CO2['totCol'])
std_CO2 = np.std(f_CO2['totCol'])
#print 'CO2 MEAN STD',np.mean(f_CO2['totCol']),np.std(f_CO2['totCol'])
con_CO2_II = ((f_CO2['totCol'] > mn_CO2-2*std_CO2) & (f_CO2['totCol'] < mn_CO2+2*std_CO2))
...
con_O2_I = ((f_O2['relRMS'] > 0.0) & (f_O2['relRMS'] < 0.36)) & \
((f_O2['wnShift'] > -5.0e-6) & (f_O2['wnShift'] < 1.5e-5)) & \
((f_O2['TOTerr'] < 9.0e22))
f_O2 = f_O2[con_O2_I]
dt_O2 = dt_O2[con_O2_I]
print 'FILTER #1 O2', f_O2.shape
# 2ND FILTER O2
mn_O2 = np.mean(f_O2['totCol'])
std_O2 = np.std(f_O2['totCol'])
#print 'O2 MEAN STD',np.mean(f_O2['totCol']),np.std(f_O2['totCol'])
con_O2_II = ((f_O2['totCol'] > mn_O2-2*std_O2) & (f_O2['totCol'] < mn_O2+2*std_O2))
f_O2 = f_O2[con_O2_II]
...
def writeXCO2txt(CO2_mat,O2_mat,XCO2):
	...
	for ii,ele in enumerate(XCO2):
		if CO2_mat['epochTime'][ii] == O2_mat['epochTime'][ii]:
			string_txt = string_txt+CO2_mat['fname'][ii]+' '+str(CO2_mat['epochTime'][ii])+' %.4f %.4f %.4e %.4e %.4e %.4e\n' % (CO2_mat['SZA'][ii],ele,CO2_mat['totCol'][ii],O2_mat['totCol'][ii],CO2_mat['TOTerr'][ii],O2_mat['TOTerr'][ii])
	min_date = dt.datetime.fromtimestamp(min(CO2_mat['epochTime']))
	max_date = dt.datetime.fromtimestamp(max(CO2_mat['epochTime']))
	file_XCO2 = open('%02i%02i-%02i%02i_XCO2_CCA.txt' % (min_date.year-2000,min_date.month,max_date.year-2000,max_date.month),'w')
	file_XCO2.write(string_txt)
	file_XCO2.close()
	print 'ARCHIVO %02i%02i-%02i%02i_XCO2_CCA.txt GENERADO' % (min_date.year-2000,min_date.month,max_date.year-2000,max_date.month)
...
print 'MIN (%.4f) MAX (%.4f) DIFF DIURNAL CYCLE: %.4f' % (min(XCO2_tshourlymean),max(XCO2_tshourlymean),max(XCO2_tshourlymean)-min(XCO2_tshourlymean))
...
plt.yticks(fontsize=f_size)
plt.xticks(XCO2_unq_wd,['Mon','Tue','Wed','Thu','Fri','Sat','Sun'],rotation=45,fontsize=f_size)
plt.ylabel('$XCO_2\/(ppm)$',fontsize=f_size)
plt.xlim([min(XCO2_unq_wd)-0.2,max(XCO2_unq_wd)+0.2])
plt.ylim([394.0,397.0])
plt.savefig('WEEKCYCLE_XCO2.jpg')
...
#plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%b-%d'))
plt.yticks(fontsize=f_size)
plt.xticks(XCO2_unq_tshr[1:-1],["%i:00" % ele for ele in XCO2_unq_tshr[1:-1]],rotation=45,fontsize=f_size)
