...
######################################################################
####################	FILENAMES AND PATH
f_path = "/home/operador/Desktop/jorge/PROFFIT"
n_CO2 = "1603-1606_CO2_EM27.txt"
n_O2 = "1603-1606_O2_EM27.txt"
...
con_CO2_I = ((f_CO2['relRMS'] > 0.0) & (f_CO2['relRMS'] < 0.003)) #& \
...
con_CO2_II = ((f_CO2['totCol'] > mn_CO2-2*std_CO2) & (f_CO2['totCol'] < mn_CO2+2*std_CO2))	### SEGUNDO FILTRO USADO ANTES DE 13/05/16
#con_CO2_II = ((f_CO2['totCol'] > mn_CO2-10*std_CO2) & (f_CO2['totCol'] < mn_CO2+10*std_CO2))
...
con_O2_I = ((f_O2['relRMS'] > 0.0) & (f_O2['relRMS'] < 0.003)) #& \
...
con_O2_II = ((f_O2['totCol'] > mn_O2-2*std_O2) & (f_O2['totCol'] < mn_O2+2*std_O2))		### SEGUNDO FILTRO USADO ANTES DE 13/05/16
#con_O2_II = ((f_O2['totCol'] > mn_O2-10*std_O2) & (f_O2['totCol'] < mn_O2+10*std_O2))
f_O2 = f_O2[con_O2_II]
...
datetime_CO2 = np.array([dt.datetime.fromtimestamp(ala) for ala in CO2_cmm['epochTime']])
date_CO2 = np.array([yly.date() for yly in datetime_CO2])
datetime_O2 = np.array([dt.datetime.fromtimestamp(ele) for ele in O2_cmm['epochTime']])
date_O2 = np.array([yly.date() for yly in datetime_O2])
...
plt.figure()
ax1 = plt.subplot(311)
plt.plot(datetime_CO2,CO2_cmm['totCol']/10**21,color='b',marker='.',linestyle='None',label=None)
...
plt.ylabel('$CO_2\/10^{21}\/(mol/cm^2)$',fontsize=f_size)
ax2 = plt.subplot(312,sharex=ax1)
plt.plot(datetime_O2,O2_cmm['totCol']/10**24,color='g',marker='.',linestyle='None',label=None)
...
plt.yticks(fontsize=f_size)
plt.xticks(fontsize=f_size)		
plt.ylabel('$O_2\/10^{24}\/(mol/cm^2)$',fontsize=f_size)
ax3 = plt.subplot(313,sharex=ax1)
plt.plot(XCO2_dt,XCO2,color='r',marker='.',linestyle='None',label=None)
#plt.plot(XCO2_dt,fourier_fun_XCO2,linestyle='-',marker='None',linewidth=3,color='k',label=None)
#plt.plot(XCO2_dt,trend_XCO2,linestyle='--',marker='None',linewidth=1,color='k',label='%.2f ppm/yr' % (coef_XCO2[1]))
#plt.errorbar(XCO2_unq_d,XCO2_dailymean,yerr=XCO2_dailystd,color=plt_color,marker='.',linestyle='None',label=None)
#plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%b'))
